...
{
    public class SG777_Line : ILHotfixEntity
    {
        ...
        {
            if (SG777Entry.Instance.GameData.ResultData.nWinGold <= 0) return;
        ...
        {
            RollContent = SG777Entry.Instance.RollContent; //转动区域
            animContent = SG777Entry.Instance.Icons; //显示库
        }
        /// <summary>
        /// 创建动画
        /// </summary>
        ...
        {
            //创建动画，先从对象池中获取
            Transform go = SG777Entry.Instance.effectPool.Find(effectName);
            if (go != null) return go.gameObject;

            go = SG777Entry.Instance.Icons.Find(effectName);
        ...
        private void CollectEffect(GameObject effect)
        {
            ...
            effect.transform.SetParent(SG777Entry.Instance.effectPool);
            effect.SetActive(false);
        }

        private class IdleState : State<SG777_Line>
        {
            public IdleState(SG777_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {

            }
        }

        private class ReceiveResultState : State<SG777_Line>
        {
            public ReceiveResultState(SG777_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            ...
            public override void OnEnter()
            {
                ...
                SG777_Struct.CMD_3D_SC_Result result = SG777Entry.Instance.GameData.ResultData;
            ...
            public override void Update()
            {
                ...
                {
                    for (int i = 0; i < SG777Entry.Instance.GameData.ResultData.cbIcon.Count; i++)
                    {
                        if (SG777Entry.Instance.GameData.ResultData.cbIcon[i] == 10)//有scatter
                        {
                            scatterlist.Add(i);
                        }
                        if (SG777Entry.Instance.GameData.ResultData.cbIcon[i] == 9)
                        {
                            freelist.Add(i);
                        }
                    }
                    ...
                    else if (freelist.Count>=6 && SG777Entry.Instance.GameData.ResultData.nFreeCount>0)
                    {
        ...
        /// 单显 轮播
        /// </summary>
        private class ShowSingleState : State<SG777_Line>
        {
            public ShowSingleState(SG777_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
        ...
        /// 总显
        /// </summary>
        private class ShowTotalState : State<SG777_Line>
        {
            public ShowTotalState(SG777_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            float timer;
            public override void OnEnter()
            {
                ...
                for (int i = 0; i < owner.lineDatas.Count; i++)
                {
                    List<int> lines = SG777_DataConfig.Lines[owner.lineDatas[i].Index];//中奖线
                    //GameObject child = owner.transform.gameObject.InstantiateChild(i);
                    SG777_Audio.Instance.PlaySound(SG777_Audio.Line);
                    ...
                    for (int j = 0; j < owner.lineDatas[i].Count; j++)
                    {
                        ...
                        if (animTrans.childCount <= 0)
                        {
                            string effectName = SG777_DataConfig.IconTable[SG777Entry.Instance.GameData.ResultData.cbIcon[hitIndex]];
            ...
            public override void Update()
            {
                ...
                if (timer >= SG777_DataConfig.lineAllShowTime)
                {
                    timer = 0;
        ...
        /// 关闭所有icon动画
        /// </summary>
        private class CloseEffectState : State<SG777_Line>
        {
            public CloseEffectState(SG777_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            bool isComplete;
            public override void OnEnter()
