...
{
    public class TJZ_Line : ILHotfixEntity
    {
        ...
        {
            if (TJZEntry.Instance.GameData.ResultData.nWinGold <= 0) return;
        ...
        {
            RollContent = TJZEntry.Instance.RollContent; //转动区域
            animContent = TJZEntry.Instance.Icons; //显示库
        }
        /// <summary>
        /// 创建动画
        /// </summary>
        ...
        {
            //创建动画，先从对象池中获取
            Transform go = TJZEntry.Instance.effectPool.Find(effectName);
            if (go != null) return go.gameObject;

            go = TJZEntry.Instance.Icons.Find(effectName);
        ...
        private void CollectEffect(GameObject effect)
        {
            ...
            effect.transform.SetParent(TJZEntry.Instance.effectPool);
            effect.SetActive(false);
        }

        private class IdleState : State<TJZ_Line>
        {
            public IdleState(TJZ_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {

            }
        }

        private class ReceiveResultState : State<TJZ_Line>
        {
            public ReceiveResultState(TJZ_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            ...
            public override void OnEnter()
            {
                ...
                TJZ_Struct.CMD_3D_SC_Result result = TJZEntry.Instance.GameData.ResultData;
            ...
            public override void Update()
            {
                ...
                {
                    for (int i = 0; i < TJZEntry.Instance.GameData.ResultData.cbIcon.Count; i++)
                    {
                        if (TJZEntry.Instance.GameData.ResultData.cbIcon[i] == 10)//有scatter
                        {
                            scatterlist.Add(i);
                        }
                        if (TJZEntry.Instance.GameData.ResultData.cbIcon[i] == 9)
                        {
                            freelist.Add(i);
                        }
                    }
                    ...
                    else if (freelist.Count>=6 && TJZEntry.Instance.GameData.ResultData.nFreeCount>0)
                    {
        ...
        /// 单显 轮播
        /// </summary>
        private class ShowSingleState : State<TJZ_Line>
        {
            public ShowSingleState(TJZ_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
        ...
        /// 总显
        /// </summary>
        private class ShowTotalState : State<TJZ_Line>
        {
            public ShowTotalState(TJZ_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            float timer;
            public override void OnEnter()
            {
                ...
                for (int i = 0; i < owner.lineDatas.Count; i++)
                {
                    List<int> lines = TJZ_DataConfig.Lines[owner.lineDatas[i].Index];//中奖线
                    ...
                    for (int j = 0; j < owner.lineDatas[i].Count; j++)
                    {
                        ...
                        if (animTrans.childCount <= 0)
                        {
                            string effectName = TJZ_DataConfig.IconTable[TJZEntry.Instance.GameData.ResultData.cbIcon[hitIndex]];
            ...
            public override void Update()
            {
                ...
                if (timer >= TJZ_DataConfig.lineAllShowTime)
                {
                    timer = 0;
        ...
        /// 关闭所有icon动画
        /// </summary>
        private class CloseEffectState : State<TJZ_Line>
        {
            public CloseEffectState(TJZ_Line owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            bool isComplete;
            public override void OnEnter()
