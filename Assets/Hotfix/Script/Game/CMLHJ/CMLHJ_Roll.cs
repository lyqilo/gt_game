...
{
    public class CMLHJ_Roll : ILHotfixEntity
    {
        ...
        protected override void Awake()
        {
            ...
            CMLHJ_Event.StopRoll += CMLHJ_Event_StopRoll;
        }
        ...
        private void CMLHJ_Event_StopRoll(bool force)
        {
            if (force)
            {
        ...
        private void ChangeResultIcon(int rollIndex)
        {
            ...
            for (int i = 0; i < iconParent.childCount-1; i++)
            {
                ...
                int iconIndex;
                iconIndex = CMLHJEntry.Instance.GameData.ResultData.cbIcon[i * 5 + rollIndex];
                ...
                {
                    iconParent.GetChild(i+1).gameObject.name = CMLHJ_DataConfig.IconTable[iconIndex];
                }
            }
        }
        ...
        /// 闲置状态
        /// </summary>
        private class IdleState : State<CMLHJ_Roll>
        {
            public IdleState(CMLHJ_Roll owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
        }
        /// <summary>
        /// 转动状态
        /// </summary>
        private class RollState : State<CMLHJ_Roll>
        {
            public RollState(CMLHJ_Roll owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            float timer;
            float startTimer;
            public override void OnEnter()
            {
                ...
                CMLHJ_Audio.Instance.PlaySound(CMLHJ_Audio.Roll);
                startTimer = 0;
                owner.rollIndex = 0;
            }
            public override void Update()
            {
                ...
                if (owner.rollIndex == owner.rollList.Count && startTimer <= CMLHJ_DataConfig.rollTime)
                {
                    //计算旋转时间，时间到就停止
                    startTimer += Time.deltaTime;
                    if (startTimer >= CMLHJ_DataConfig.rollTime)
                    {
                        startTimer = 0;
                        CMLHJ_Event.DispatchStopRoll(false);
                    }
                }
            }
        }
        ...
        /// 停止状态
        /// </summary>
        private class StopState : State<CMLHJ_Roll>
        {
            public StopState(CMLHJ_Roll owner, HierarchicalStateMachine hsm) : base(owner, hsm)
            {
            }
            int stopIndex = 0;
            float stopTimer = 0;
            ...
            private void CalculateStop()
            {
                if (stopIndex < owner.rollList.Count)
                {
                    ...
                    if (stopTimer >= CMLHJ_DataConfig.rollStopInterval)
                    {
                        stopTimer = 0;
                        //TODO 换正式结果图片
                        stopIndex++;
                        ...
                        CMLHJ_Audio.Instance.PlaySound(CMLHJ_Audio.Stop);
                        if (stopindex==4)
                        {
                            PopStopRaw();
                        }
            ...
            private void PopStopRaw()
            {
                ...
                CMLHJ_Event.DispatchRollComplete();
            }
        }
    }
}