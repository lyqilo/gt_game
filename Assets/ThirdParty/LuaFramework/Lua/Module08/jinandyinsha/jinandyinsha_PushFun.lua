jinandyinsha_PushFun = {};
local self = jinandyinsha_PushFun;

-- 创建数字Img显示对象
function jinandyinsha_PushFun.CreatShowNum(father, getnumstr,numpathstr,isdanw,w,isagin,aginw,aginx,xis) 
    ...
    for i = 1, string.len(numstr) do
        ...
        item = BirdsAndBeast_GameData.iconres.transform:Find(numpathstr..prefebnum);
        if (item == nil) then
            return
        end
    ...
    elseif isagin == 3 then
        father.transform.localPosition = Vector3.New(aginx-kw*xisu,sy,0);
    end
end

function jinandyinsha_PushFun.repaceNum(args)
    if args == "." then
        return 10;
    elseif args == "w" then
        return 11;
...
--显示金钱 只是万和亿 用 w y
function jinandyinsha_PushFun.showNumberText2(numInt)
