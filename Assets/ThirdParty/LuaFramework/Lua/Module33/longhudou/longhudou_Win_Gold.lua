...
function longhudou_Win_Gold.setPer(args)
   self.init();
   self.per = args;
...
function longhudou_Win_Gold.init(args)
    self.guikey = "cn";
    self.per = nil;
    self.winGoldroll = nil;
    self.addgold = 0;--增加的钱
...
function longhudou_Win_Gold.winGoldRollCom(obj,args)
    self.per.gameObject:SetActive(false);
   if self.isclose == false then
      coroutine.start(function (args)
         ...
         longhudou_Event.dispathEvent(longhudou_Event.xiongm_gold_roll_com,self.ftar);
...
function longhudou_Win_Gold.winGoldRollIng(obj,args)
    longhudou_PushFun.CreatShowNum(self.moneycont,args,"win_",false,82,2,430,-200);
end

function longhudou_Win_Gold.addEvent()
   longhudou_Event.addEvent(longhudou_Event.xiongm_show_win_gold,self.showWinGold,self.guikey);
end

function longhudou_Win_Gold.isDispathGameOver()
...
function longhudou_Win_Gold.showWinGold(args)
   ...
   self.per.gameObject:SetActive(true);
    longhudou_PushFun.CreatShowNum(self.moneycont,args.data.addgold,"win_",false,82,2,430,-200);
   coroutine.start(function()
        coroutine.wait(1);
        if longhudou_Socket.isongame==false then
           return;
...
function longhudou_Win_Gold.doShowWinGold(args)
...
function longhudou_Win_Gold.removeEvent()
