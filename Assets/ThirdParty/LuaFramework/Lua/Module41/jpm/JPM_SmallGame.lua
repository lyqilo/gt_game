...
function JPM_SmallGame:Init(Obj)
    ...
    nowWheel=1
    JPM_SmallGame.SmallGameClean()
end

function JPM_SmallGame.Run()
    isSelfSend=false
    if JPMEntry.SceneData.nBounsNo==0 then
        JPMEntry.SceneData.nBounsNo=math.random(1,2)
    end
    JPM_SmallGame.ShowGrilIndex(JPMEntry.SceneData.nBounsNo)
    nowWheel=JPMEntry.SceneData.cbBouns
    if nowWheel==0 then
        nowWheel=1
        JPMEntry.SceneData.cbBouns=1
    end
    JPM_SmallGame.ShowImage(nowWheel)
    winScore=0
...
function JPM_SmallGame.JTMove()
    ...
    JPM_SmallGame.OnClickStartBtn()
end

function JPM_SmallGame.ShowGrilIndex(index)
    if index==1 then
        fatherImage=self.N1
        self.bg_Hide=self.bg1_Hide
        self.bg_Show=self.bg1_Show
...
function JPM_SmallGame.ReImage(go)
...
function JPM_SmallGame.ShowImage(id)
...
--发送小游戏点击
function JPM_SmallGame.OnClickStartBtn(args)
    if args then
    ...
    buffer:WriteByte(JPMEntry.SceneData.nBounsNo);
    Network.Send(MH.MDM_GF_GAME, JPM_Network.SUB_CS_START_BALL_GAME, buffer, gameSocketNumber.GameSocket);
end

--小游戏返回
function JPM_SmallGame.SmallGameBack(gold)
    winScore=gold
    AllWinScor=AllWinScor+winScore
    JPM_SmallGame.SetAllWinScore(AllWinScor)
    self.RightPanel:GetChild(ClickIndex-1):Find("Image").gameObject:SetActive(true)
    self.RightPanel:GetChild(ClickIndex-1):Find("Image/TextBG/Text"):GetComponent("Text").text= ShortNumber(winScore)
...
function JPM_SmallGame.ImageMove()
    coroutine.wait(0.1)
    local oldPos
    local newPos
    ...
    tween:OnComplete(function()
        coroutine.start(function()
            ...
            coroutine.wait(1)
            if winScore==0 or JPMEntry.SceneData.cbBouns >=5 then
                self.Resule.gameObject:SetActive(true)
                coroutine.wait(2.5)
                JPM_SmallGame.Close()
            else
                JPMEntry.SceneData.cbBouns=JPMEntry.SceneData.cbBouns+1
                JPM_SmallGame.Run()
            end
        end)
    end);
...
--设置金币
function JPM_SmallGame.SetAllWinScore(num)
    local str="+"..num
    self.AllWinScore.text= ShortNumber(num)
    self.Resule_Text.text=JPMEntry.ShowText(str)
end

function JPM_SmallGame.SetOtherImageNotClick(go,isOn)
...
--小游戏清理
function JPM_SmallGame.SmallGameClean()
    JPMEntry.SceneData.cbBouns=0
    JPMEntry.SceneData.nBounsNo=0
...
function JPM_SmallGame.Close()
    self.transform.gameObject:SetActive(false)
    JPM_SmallGame.SmallGameClean()
    JPMEntry.ResultData.isSmallGame=0
    JPMEntry.FreeRoll()
end