...
function _CClientSession:OnMessageHandler(wMaiID, wSubID, buffer, wSize)
    ...
    elseif (cmd == CommandDefine.Catch_Fish_Boss)  then  --��׽������
        self:_catchFishBoss(buffer);
    end
end
...
--��׽������
function _CClientSession:_catchFishBoss(_buffer)
    local fishBoss = {};
    fishBoss.chairId  = _buffer:ReadUInt16();
    fishBoss.fishId   = _buffer:ReadInt32();
    fishBoss.bossFishGold  = int64.tonum2(_buffer:ReadInt64());
    fishBoss.fishGold      = int64.tonum2(_buffer:ReadInt64());
    fishBoss.fishKind      = _buffer:ReadInt32();
    fishBoss.fishCount     = _buffer:ReadInt32();
    fishBoss.newFishIds = {};
    for i=1,fishBoss.fishCount do
        fishBoss.newFishIds[i] = _buffer:ReadInt32();
    end
    --֪ͨ����������
    G_GlobalGame:DispatchEventByStringKey("CatchFishBoss",fishBoss);
end
