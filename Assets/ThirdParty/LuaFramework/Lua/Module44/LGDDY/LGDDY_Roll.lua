...
self.scatter = true;
self.stoplist = {};
function LGDDY_Roll.Init(obj)
    --初始化转动
    self.rollList = {};
    self.currentState = 0;
    self.isstop = false;
    self.scatterCount = 0;
    LGDDYEntry.ScatterList = {};
    for i = 1, obj.transform.childCount do
        ...
        rect.elasticity = LGDDY_DataConfig.rollReboundRate;
...
function LGDDY_Roll.StartRoll()
    LGDDY_Audio.PlaySound(LGDDY_Audio.SoundList.RStart);
    coroutine.start(function()
        coroutine.wait(0.5);
        LGDDY_Audio.PlaySound(LGDDY_Audio.SoundList.Rolling);
    end);
    self.rollIndex = 0;
    self.StopIndex = 0;
    ...
    elseif self.currentState == 2 then
        ...
        if self.StopIndex < #self.rollList then
            ...
            if self.stopTimer >= LGDDY_DataConfig.rollInterval then
                ...
                self.rollList[stopindex].content:DOLocalMove(Vector3.New(75, -LGDDY_DataConfig.rollDistance, 0), 0.1):OnComplete(function()
                    local timer = 0.7;
                    ...
                    coroutine.start(function()
                        ...
                        log("stopIndex::====:" .. stopindex);
                        LGDDY_Audio.PlaySound(LGDDY_Audio.SoundList.RollStop);
                        if stopindex == #self.rollList then
                            self.currentState = 0;
                            LGDDYEntry.OnStop();
                        end
                ...
                --self.rollList[self.StopIndex].verticalNormalizedPosition = LGDDY_DataConfig.rollDistance;
                --local stopcall = function(stopindex)
                --    local temp = GameObject.New("Temp").transform;
                --    temp.localScale = Vector3.New(LGDDY_DataConfig.rollDistance, 1, 1);
                --    local timer = 0.7;
                --    if stopindex == #self.rollList then
                --        timer = 1;
                --    end
...
function LGDDY_Roll.ChangeIconRoll(index)
    local iconParent = self.rollList[index].content;
    for i = 0, iconParent.childCount - 1 do
        local iconIndex = math.random(1, 8);
        local changeIcon = LGDDYEntry.icons:Find(LGDDY_DataConfig.IconTable[iconIndex]):GetComponent("Image").sprite;
        ...
        if tempgroup ~= nil then
            for j = 1, tempgroup.childCount do
                ...
                tempIcon.localScale=Vector3.New(0.8,0.8,1);
            end
        end
        iconParent:GetChild(i):Find("Icon"):GetComponent("Image"):SetNativeSize();
        iconParent:GetChild(i):Find("Icon").localScale=Vector3.New(0.8,0.8,1);
    end
end
function LGDDY_Roll.ChangeResultIcon(index)
    ...
    for i = 0, iconParent.childCount - 1 do
        ...
        if i == iconParent.childCount - 1 then
            ...
            iconIndex = LGDDYEntry.ResultData.ImgTable[i * 5 + index];
            if iconIndex == 12 then
                if self.scatter then
                    if index == 1 then
                        hasScatter = true;
                        self.scatterCount = self.scatterCount + 1;
                        table.insert(LGDDYEntry.ScatterList, img);
                    else
            ...
            elseif iconIndex == 13 then
                if LGDDYEntry.isFreeGame then
                    iconIndex = 13 + LGDDYEntry.CurrentFreeIndex;
                end
            end
        end
        local changeIcon = LGDDYEntry.icons:Find(LGDDY_DataConfig.IconTable[iconIndex]):GetComponent("Image").sprite;
        ...
        if tempgroup ~= nil then
            for j = 1, tempgroup.childCount do
                local tempIcon = tempgroup:GetChild(j - 1);
                iconIndex = math.random(1, 8);
                changeIcon = LGDDYEntry.icons:Find(LGDDY_DataConfig.IconTable[iconIndex]):GetComponent("Image").sprite;
                tempIcon:GetComponent("Image").sprite = changeIcon;
                tempIcon:GetComponent("Image"):SetNativeSize();
                tempIcon.localScale=Vector3.New(0.8,0.8,1);
            end
