...
function _CHelpPanel:ctor(_parent)
    ...
    Tools.LuaScript.AddClick(closeBtn.gameObject,handler(self,self._onCloseClick));
    self.closeBtn = closeBtn;
    self.startPos = Vector3.zero;
    Tools.LuaScript.AddDrag(self.transform:Find("Mask").gameObject,handler(self,self.onBeginDrag),
        handler(self,self.onEndDrag));
    ...
    self._pagePointsCtrl   = { 
        ...
        RenewPages = function(self,_totalPage)
            if self.count~=_totalPage then
                if self.realCount>=_totalPage then
                    ...
                    local point;
                    local img;
                    local THandler = Tools.Handler.Create;
                    --
                    for i = self.realCount+1,_totalPage do
                        ...
                        img:SetNativeSize();
                        self.points[i] = point;
                        Tools.LuaScript.AddClick(gameObject,THandler(self,self._trunToPage,i));
                    end
        ...
        _trunToPage = function (self ,index)
    ...
    self._pagePointsCtrl:RenewPages(3);
end

function _CHelpPanel:onBeginDrag(go, args)
   self.startPos = args.position;
end


function _CHelpPanel:onEndDrag(go, args)
...
function _CHelpPanel:_onChangePage(_index)
    ...
    self.curPage:SetActive(true);end
