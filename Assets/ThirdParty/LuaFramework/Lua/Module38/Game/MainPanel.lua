...
local function Include()
    ...
    GameRequire__ = function(name, _packetname)
        ...
        if __LoadFileTable[realName]==true then
            ...
            return reimport(realName);
        end
    end

    GamePacketName__("Module38.Game");
    GameRequire__("Tools");
...
function MainPanel:Awake(obj)
    ...
    Event.AddListener(HallScenPanel.ReconnectEvent, self.OnBackGame);
    ...
    G_GlobalGame._canvasPanel.localPosition = VECTOR3ZERO
    ...
    G_GlobalGame._canvasPanel:Find("Layers"):GetComponent("RectTransform").sizeDelta = Vector2.New((Screen.width / Screen.height) * 750 + 20, 750 + 20);
    G_GlobalGame._canvasPanel:Find("Layers/ContentLayer"):GetComponent("RectTransform").sizeDelta = Vector2.New((Screen.width / Screen.height) * 750 + 20, 750 + 20);
    ...
    do
        ...
        --G_GlobalGame._canvasPanelGO:SetActive(false);
    end
    --G_GlobalGame:openLog();
end

function MainPanel:OnDestroy()
    Event.RemoveListener(HallScenPanel.ReconnectEvent, self.OnBackGame);
end
...
function MainPanel:HideOtherUI()
     G_GlobalGame._canvasPanelGO:SetActive(true);
     --先隐藏他们的UI界面
     G_GlobalGame._otherUI:SetActive(false);
end
...
--            local time = ( os.clock() - real);
--            if time>=0.005 then
--                error("clientSession:update:" .. time);
--            end
--            real = os.clock()
--        end
