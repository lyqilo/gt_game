xiangqi_PushFun = {};
local self = xiangqi_PushFun;

-- 创建数字Img显示对象
function xiangqi_PushFun.CreatShowNum(father, getnumstr,numpathstr,isdanw,w,isagin,aginw,aginx) 
    ...
    for i = 1, string.len(numstr) do
        ...
        item = xiangqi_Data.numres.transform:Find(numpathstr..prefebnum);
...
function xiangqi_PushFun.repaceNum(args)
    if args == "." then
        return 10;
    elseif args == "w" then
        return 11;
...
--显示金钱 只是万和亿 用 w y
function xiangqi_PushFun.showNumberText2(numInt)
