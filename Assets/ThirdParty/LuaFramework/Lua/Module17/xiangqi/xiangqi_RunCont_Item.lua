xiangqi_RunCont_Item = {};
local self = xiangqi_RunCont_Item;

function xiangqi_RunCont_Item:new()
   local go = {};
   self.guikey = "cn";
...
function xiangqi_RunCont_Item:init()
...
function xiangqi_RunCont_Item:runStop()    
    self.iscom = true;
end

function xiangqi_RunCont_Item:setOpenPos()
...
function xiangqi_RunCont_Item:setPer(runargs,showargs,cindex,lihuosp)
   self:init();
   ...
   self.showdata = xiangqi_Config.defalut_show_img[self.curindex];
   self.opendata = xiangqi_Config.defalut_show_img[self.curindex];
   self:addEvent();
end

function xiangqi_RunCont_Item:setSonItem(per,runitem)
    self.sonItem = per;
    self.runSonItem = runitem;
end

function xiangqi_RunCont_Item:addEvent()
    xiangqi_Event.addEvent(xiangqi_Event.xiangqi_over,xiangqi_Event.hander(self,self.gameOver),self.guikey);
end

function xiangqi_RunCont_Item:gameOver(args)
    ...
    local waittime = xiangqi_Config.runstoptimer_config[self.curindex];
    if self.maxtimer>8 then
        xiangqi_Socket.playaudio("freespeed",false,false);
        waittime = waittime/2; 
    end
    self.opendata = {};
    self.showdata = {};
    ...
    self.playStop = false;
    for i=1,3 do
        openimg =  xiangqi_Data.openImg[self.curindex+(3-i)*5];
        self.opendata[i] = openimg;
        self.showdata[i] = openimg;
        if openimg == xiangqi_Config.general then
...
function xiangqi_RunCont_Item:randImg()
   ...
   if xiangqi_Data.byFreeCnt==0 then
      config = xiangqi_Config.rand_1;
   else
     config = xiangqi_Config.rand_2;
   end
   for i=midindex,endindex do
     ...
     if sonfig==nil then
       ...
       if sonfig~=0 then
          self:setRunImg(self.runtabel[i],xiangqi_Config.resimg_config[sonfig.src].normalimg);
       end 
   end
end

--设计显示
function xiangqi_RunCont_Item:setShow()
   local item = nil;
   local idata = nil;
   for i=1,3 do
       idata = self.opendata[i];
       item = self.showtabel[i];
       item:setImg(xiangqi_Config.resimg_config[idata].normalimg);
   end
end

function xiangqi_RunCont_Item:setRunShow(args)
   local item = nil;
   local idata = nil;
   for i=1,3 do
       idata = self.opendata[i];
       item = self.runtabel[i];
       self:setRunImg(item,xiangqi_Config.resimg_config[idata].normalimg);
   end
end

function xiangqi_RunCont_Item:startRun(ags)
   if self.curindex == 1  then
      xiangqi_Data.isruning = true;
   end
    self.lastjul = 0;
    self.maxtimer = 7;
    self.jiantimer = 0.4;
...
--开奖结果
function xiangqi_RunCont_Item:setOpenWin()
   local item = nil;
   local idata = nil;
...
function xiangqi_RunCont_Item:randFun(randtabel)
    local reslut = 0;   
...
--生成里面的图片
function xiangqi_RunCont_Item:createSonItem(num)
   local item = nil;
   self.createnum = num;
   for i = 1,num do
   ...
   for a = 1,3 do
      item = xiangqi_Run_Soon_Item:new();
      ...
      xiangqi_Data.allshowitem[self.curindex+(3-a)*5] = item;
   end  
   self:randImg();   
   self:setShow();
   self:setRunShow(); 
...
function xiangqi_RunCont_Item:setRunImg(item,imgname)
   local itemimg = nil;
   if xiangqi_Data.icon_res==nil then
      itemimg = xiangqi_Data.numres.transform:Find(imgname).gameObject;
   else
      itemimg = xiangqi_Data.icon_res.transform:Find(imgname).gameObject;
   end
...
function xiangqi_RunCont_Item:deFault()
   local item = nil;
...
function xiangqi_RunCont_Item:Update()
...
function xiangqi_RunCont_Item:clientRun()
    local pos = nil;
    local py = 0;
    ...
    for i=1,len do
         ...
         if py< (self.starty -self.sh)  then
            ...
            if xiangqi_Data.istihuan==true and xiangqi_Data.isPalyFreeAnima==false then
               self:replaceImg(i);
            end
...
function xiangqi_RunCont_Item:replaceImg(args)
  if args==4 and self.curindex==5 then
     xiangqi_Data.istihuan=false;
  end
  local randnum = math.random(1,#xiangqi_Config.rand_2);
  self:setRunImg(self.runtabel[args],xiangqi_Config.resimg_config[xiangqi_Config.rand_2[randnum].src].normalimg);
end

function xiangqi_RunCont_Item:serverRun()
   local pos = nil;
   local py = 0;  
    if self.isrun == false then
       return;
    ...
    if self.runStopLen>0 then
       ...
       if self.runStopLen<100 then
           if self.playStop == false then
              self.playStop = true;
              if self.isfreesound==true and xiangqi_Data.istssound==true then
                  xiangqi_Socket.playaudio("freeiconstop",false,false);
               else
                  xiangqi_Socket.playaudio("normalstop",false,false);
              end               
           end
       end      
    end
    ...
    if self.isrun == false then
       ...
       if self.curindex==5 then
          ...
          xiangqi_Data.isruning = false;
       end
        xiangqi_Event.dispathEvent(xiangqi_Event.xiongm_run_com,self.curindex);
    end
end
