...
function xiangqi_NumRolling:New()
   local go = {};
...
-- callcom 运动完成
-- callupdate 中间更新
function xiangqi_NumRolling:setdata(snum,enum,dtimer)
...
function xiangqi_NumRolling:getisrun()
  return self.isrun;
end

function xiangqi_NumRolling:setfun(callobj,callcom,callupdate)
   self.callobj = callobj;
   self.callcom = callcom;
   self.callupdate = callupdate;
end

function xiangqi_NumRolling:update()
