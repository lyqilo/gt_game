xiangqi_RunComp = {};
local self = xiangqi_RunComp;

function xiangqi_RunComp:new()
   local go = {};
   self.guikey = "cn";
...
function xiangqi_RunComp:runStop()    
    self.iscom = true;
end

function xiangqi_RunComp:setOpenPos()
...
function xiangqi_RunComp:setPer(runargs)
   self:init();
...
function xiangqi_RunComp:setSonItem(per)
    self.sonItem = per;
end

function xiangqi_RunComp:randImg()
     ...
     if sonfig==nil then
       ...
       if sonfig~=0 then
          if xiangqi_Data.icon_res==nil then
             self.runtabel[i].transform:Find("icon").gameObject:GetComponent("Image").sprite = xiangqi_Data.numres.transform:Find(xiangqi_Config.resimg_config[sonfig.src].normalimg).gameObject:GetComponent("Image").sprite;
          else
            self.runtabel[i].transform:Find("icon").gameObject:GetComponent("Image").sprite = xiangqi_Data.icon_res.transform:Find(xiangqi_Config.resimg_config[sonfig.src].normalimg).gameObject:GetComponent("Image").sprite;
          end
...
function xiangqi_RunComp:randFun(randtabel)
    local reslut = 0;   
...
--生成里面的图片
function xiangqi_RunComp:createSonItem(num)
   local item = nil;
   self.createnum = num;
   for i = 1,num do
...
function xiangqi_RunComp:deFault()
   local item = nil;
...
function xiangqi_RunComp:Update()
...
function xiangqi_RunComp:clientRun()
   local pos = nil;
    local py = 0;
...
function xiangqi_RunComp:serverRun()
   local pos = nil;
   local py = 0;
