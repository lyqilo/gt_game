TBNNSCInfo = { };
local self = TBNNSCInfo;
local IsHomeBack=false;
--初始化消息结构体
function TBNNSCInfo.AddGameMessage()
...
function TBNNSCInfo.OnHandleMessage()
    --error(" -- 重新连接");
    if self.ReLogon then return end
    self.ReLogon = true;
    
    if self.LogonNum > 3 then self.RoomBreakQuit("connect failed") self.ReLogon = false; self.LogonNum = 0; return end
...
--加载帮助界面
function TBNNSCInfo.OpenHelp()
  -- LoadAssetCacheAsync('module18/game_niuniuhelptwo', 'Help_NiuNiu', TBNNPanel.CreatHelp);
  TBNNPanel.CreatHelp(Module55Panel.Pool("Help_NiuNiu"));
end


--用户登陆
function TBNNSCInfo.gamelogon()
    error("用户登陆");
    IsHomeBack = false;
    self.AddGameMessage();
    local data =
    ...
    local buffer = SetC2SInfo(TBNNMH.CMD_GR_LogonByUserID, data);
...
-- 登陆完成消息
function TBNNSCInfo.LogonOverMethod(wMain, wSubID, buffer, wSize)
    if (65535 == TBNNMH.MySelf_ChairID) then
        --error(" -- 不在桌子上");
...
--用户准备
function  TBNNSCInfo.PlayerPrepare()    
    local Data = { [1] = 0, }
    -- 旁观标志 必须等于0
    local buffer = SetC2SInfo(TBNNMH.CMD_GF_Info, Data);
    Network.Send(MH.MDM_ScenInfo, MH.SUB_GF_INFO, buffer, gameSocketNumber.GameSocket);
    error("=========用户准备===========")
end

--入座完成
function TBNNSCInfo.OnInSitOver(wMaiID, wSubID, buffer, wSize)
    if(wSize > 0) then
        error("=============入座失败=============");
...
--登陆成功
function TBNNSCInfo.OnGameLogonWin(wMaiID, wSubID, buffer, wSize)
  error("=============登陆成功============= " .. TableUserInfo._9wChairID); 
    TBNNMH.MySelf_ChairID=TableUserInfo._9wChairID;
end

-- 登陆失败消息
function TBNNSCInfo.LogonFailed(wMain, wSubID, buffer, wSize)
    error("=============登陆失败=============");
    --error(buffer:ReadString(wSize));
    Network.OnException(buffer:ReadString(wSize),1);
end
...
function TBNNSCInfo.OnHandleGameInfo(wMain, wSubID, buffer, wSize)
    error("=============游戏消息================");
    local id = string.gsub(wSubID, wMain, "");
    id = tonumber(id);
    error("=============id================"..id);

    TBNNPanel.GameInfo(id, buffer)
end

function TBNNSCInfo.WaitTimeSetInfo()
    coroutine.wait(1);
    IsHomeBack=false;
end

-- 场景消息
function TBNNSCInfo.OnHandleScenInfo(wMain, wSubID, buffer, wSize)
    --error("接受场景消息")
    error("=============场景消息=============wSize==="..wSize);
    TBNNMH.IsBankerLeave=false;
    local ScenInfoTable = { };--场景消息
    
    TBNNMH.Game_State = buffer:ReadByte();
    error("=============游戏状态================" .. TBNNMH.Game_State);
    table.insert(ScenInfoTable, TBNNMH.Game_State);
    ...
    -- 庄家座位号
    --TBNNMH.Banker_ChairID = -1
    --error("=============庄家座位号================" .. TBNNMH.Banker_ChairID);
    --table.insert(ScenInfoTable, TBNNMH.Banker_ChairID);
    ...
    --for i = 1, TBNNMH.GAME_PLAYER do
       ...
       error("=============牛牛点数================" .. byNiuNiuPoint);
        ...
        TBNNMH.lowChip = buffer:ReadInt32();
        logYellow("初始化底注=="..TBNNMH.lowChip)
    ...
    --end
    error("场景消息")
    TBNNPanel.InitScen(ScenInfoTable);
end

--表情--***
function TBNNSCInfo.OnBiaoAction(wMain, wSubID, buffer, wSize)
    local sChairId = buffer:ReadUInt16();
    local sIndex = buffer:ReadByte();
    local posnum =(sChairId - TBNNMH.MySelf_ChairID);
...
-- 用户进入
function TBNNSCInfo.OnPlayerEnter(wMain, wSubID, buffer, wSize)
    --logYellow("通知用户进入桌子=================");
    local data = { };
    data = TableUserInfo;
    local newdata =
    ...
    TBNNMH.PlayerScore=newdata._7wGold;
    NiuNiuUserInfoTable[newdata._9wChairID + 1] = newdata;
    if newdata._1dwUser_Id == SCPlayerInfo._01dwUser_Id then 
        TBNNMH.MySelf_ChairID = newdata._9wChairID 
    end
    local posnum =(newdata._9wChairID-TBNNMH.MySelf_ChairID);
    if posnum <0 then posnum = posnum + TBNNMH.GAME_PLAYER end
    posnum=posnum+1;
...
--用户离开
function TBNNSCInfo.OnPlayerLeave(wMain, wSubID, buffer, wSize)
    --error("通知用户已经离开桌子=================");
    local data = { };
    data = TableUserInfo;
    local newdata =
    ...
    local posnum =(leaverid-TBNNMH.MySelf_ChairID);
    if posnum <0 then posnum = posnum + TBNNMH.GAME_PLAYER end
    posnum=posnum+1;
    --TBNNPanel.SetSta(posnum,data._9wChairID + 1,data._1dwUser_Id);
    NiuNiuAllPlayerTable[posnum]:PlayerLeave();
    NiuNiuUserInfoTable[data._9wChairID + 1] = TBNNMH.Start_User_Data;
end

--用户状态
function TBNNSCInfo.OnPlayerStatus(wMain, wSubID, buffer, wSize)
    local data = { };
    data = TableUserInfo;
    local newdata =
    {
...
--用户分数
function TBNNSCInfo.OnPlayerScore(wMain, wSubID, buffer, wSize)
    error("用户分数")
    local data = { };
    data = TableUserInfo;
    local newdata =
    ...
    NiuNiuUserInfoTable[data._9wChairID + 1] = newdata;
    TBNNMH.PlayerScore=newdata._7wGold;
    if TBNNMH.IsOver and TBNNMH.Banker_ChairID ~= newdata._9wChairID then
        local posnum =(newdata._9wChairID-TBNNMH.MySelf_ChairID);
        if posnum <0 then posnum = posnum + TBNNMH.GAME_PLAYER end
        posnum=posnum+1;
...
-- 退出游戏
function TBNNSCInfo.OnGameQuit(wMain, wSubID, buffer, wSize)
    if NiuNiu_IsPlayingGame then
        TBNNPanel.ShowHintInfo();
    else
        TBNNPanel.SureBtnOnClick()
    end
end

--换桌
function TBNNSCInfo.OnChageTable(wMain, wSubID, buffer, wSize)
end

-- 断线消息
function TBNNSCInfo.OnRoomBreakLine(wMain, wSubID, buffer, wSize)
    local msgtype = buffer:ReadUInt16();
    local str = buffer:ReadString(buffer:ReadUInt16());
    self.RoomBreakQuit(str)
    --Network.OnException(buffer:ReadString(wSize), 1);
    --coroutine.start(TBNNPanel.DestoryAb)
end

--消息提示
function TBNNSCInfo.RoomBreakQuit(str)
...
function TBNNSCInfo.OnStopGame()
    error("TBNNSCInfo.OnStopGame");
    self.HomeStartTime = Util.TickCount;
end

-- Home键返回
function TBNNSCInfo.HomeBackGame()
    --error("清理桌面，不接受消息");
...
function TBNNSCInfo.ReLoadGame()
    --error("TBNNSCInfo.ReLoadGame()");
    TBNNPanel.ClearChip();
    self.ReLogon = false;
