...
function WLZB_Audio.Init()
    ...
    self.pool.transform:SetParent(WLZBEntry.MainContent:Find("Content"));
end
function WLZB_Audio.PlayBGM(mode)
    --播放bgm
    self.pool:Stop();
    local rc = AllSetGameInfo._5IsPlayAudio;
    local audioclip = nil;
    if mode == nil then
        audioclip = WLZBEntry.soundList:Find(self.SoundList.BGM):GetComponent("AudioSource");
    else
        audioclip = WLZBEntry.soundList:Find(mode):GetComponent("AudioSource");
    end
    self.pool.clip = audioclip.clip;
    self.pool:Play();
end

function WLZB_Audio.ResetEffect()
...
function WLZB_Audio.PlaySound(soundName, time)
    ...
    local obj = WLZBEntry.soundList:Find(soundName);
    if obj == nil then
        error("没有找到该音效");
        return ;
    end
