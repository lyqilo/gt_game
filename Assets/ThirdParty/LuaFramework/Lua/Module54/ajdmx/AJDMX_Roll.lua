...
function AJDMX_Roll.Init(obj)
    --初始化转动
    self.rollList = {};
    AJDMXEntry.scatterList = {};
    self.currentState = 0;
    self.scatterCount = 0;
    self.isstop = false;
    for i = 1, obj.transform.childCount do
        ...
        rect.elasticity = AJDMX_DataConfig.rollReboundRate;
        table.insert(self.rollList, rect);
    end

    for i=1,obj.transform.childCount do
        AJDMX_Roll.ChangeIconRoll(i)
    end
end
function AJDMX_Roll.StartRoll()

    AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.rollBegin5); 
    self.rollIndex = 0;
    self.StopIndex = 0;
    self.scatterCount = 0;
    self.timer = AJDMX_DataConfig.rollInterval;
    self.stopTimer = 0;
    self.startTimer = 0;
    self.currentState = 1;
    self.lastLight = nil;
    AJDMXEntry.scatterList = {};
    AJDMXEntry.bjjList = {};
    AJDMXEntry.bjjImgList = {};
    self.isstop = false;
    self.freeCount=0
    self.addSpeedEffectIndex = 0;

    log("timer:" .. AJDMX_DataConfig.rollTime)
end
function AJDMX_Roll.StopRoll()
    if self.isstop then
        return ;
    end
    local stop = function()
        ...
        self.stopTimer = AJDMX_DataConfig.rollInterval
        self.currentState = 2;
        self.isstop = true;
...
function AJDMX_Roll.Update()
    ...
    elseif self.currentState == 1 then
        ...
        for i = 1, self.rollIndex do
            self.rollList[i].verticalNormalizedPosition = self.rollList[i].verticalNormalizedPosition + Time.deltaTime * AJDMX_DataConfig.rollSpeed;--旋转
        ...
        if self.startTimer <= AJDMX_DataConfig.rollTime then
            --计算旋转时间，时间到就停止
    ...
    elseif self.currentState == 2 then
        ...
        if self.StopIndex < #self.rollList then
            --计算转动间隔
            local stoptime = AJDMX_DataConfig.rollInterval;
            if self.isstop then
            ...
            if self.stopTimer >= stoptime then
                if self.addSpeedEffectIndex > 0 then
                    if self.stopTimer < AJDMX_DataConfig.addSpeedTime then
                        return ;
                    end
                end
                self.stopTimer = 0;
                ...
                self.rollList[self.StopIndex].verticalNormalizedPosition = AJDMX_DataConfig.rollDistance;
...
function AJDMX_Roll.AddSpeed()
    ...
    --         return ;
    --     end
    --     -- self.lastLight = AJDMXEntry.CSGroup:GetChild(self.addSpeedEffectIndex):Find("Light").gameObject;
    --     -- self.lastLight:SetActive(true);
    --     --TODO 播放加速音效,延时销毁
    --     AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.speed1, AJDMX_DataConfig.addSpeedTime);
    -- end
...
function AJDMX_Roll.SingleRollStop(index)
    --TODO 音效
    if self.isstop then
        if index==#self.rollList then
            AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.mati_01); 
        end 
    else
        AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.mati_01);
    end
end
function AJDMX_Roll.ChangeIconRoll(index)
    local iconParent = self.rollList[index].content;
    for i = 0, iconParent.childCount - 1 do
        local iconIndex = math.random(1, 11);
        local changeIcon = AJDMXEntry.icons:Find(AJDMX_DataConfig.IconTable[iconIndex]):GetComponent("Image").sprite;
...
function AJDMX_Roll.ChangeResultIcon(index)
    --切换正式Icon
    local iconParent = self.rollList[index].content;
    for i = 0, iconParent.childCount - 1 do
        ...
        if i == iconParent.childCount - 1 then
            ...
            iconIndex = AJDMXEntry.ResultData.ImgTable[i * 5 + index];
            if iconIndex == 10 then
                ...
                if self.scatterCount == 2 then
                    AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.freeBoundS1);
                elseif self.scatterCount == 2 then
                     AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.freeBoundS2);
            ...
            if iconIndex==12 then
                ...
                if self.freeCount>=2 and index==5 then
                    AJDMX_Audio.PlaySound(AJDMX_Audio.SoundList.freeBoundS3);
                end
            end
        end
        local changeIcon = AJDMXEntry.icons:Find(AJDMX_DataConfig.IconTable[iconIndex+1]):GetComponent("Image").sprite;
        img.sprite = changeIcon;
        img:SetNativeSize();
        local tempIcon = iconParent:GetChild(i):Find("Temp");
        if tempIcon ~= nil then
            iconIndex = math.random(2, 11);
            changeIcon = AJDMXEntry.icons:Find(AJDMX_DataConfig.IconTable[iconIndex]):GetComponent("Image").sprite;
            tempIcon:GetComponent("Image").sprite = changeIcon;
            tempIcon:GetComponent("Image"):SetNativeSize();
        end
    end
