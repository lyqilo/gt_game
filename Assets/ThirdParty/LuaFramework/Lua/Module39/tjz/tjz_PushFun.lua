tjz_PushFun = {};
local self = tjz_PushFun;

-- 创建数字Img显示对象
function tjz_PushFun.CreatShowNum(father, getnumstr,numpathstr,isdanw,w,isagin,aginw,aginx) 
    ...
    for i = 1, string.len(numstr) do
        ...
        item = tjz_Data.numres.transform:Find(numpathstr..prefebnum);
        itemsize = item.transform:GetComponent("RectTransform").sizeDelta;
        if kw ~= 0 then
            klx = klx+itemsize.x/2+jianju;
        else
            rc = itemsize.x/2;
        end
        kw = kw + itemsize.x;
    ...
    elseif isagin == 2 then
        father.transform.localPosition = Vector3.New(aginx+(aginw-kw)/2+rc,sy,0);
    elseif isagin == 3 then
        father.transform.localPosition = Vector3.New(aginx+(aginw-kw),sy,0);
    end
end

function tjz_PushFun.CreatShowNum2(father, getnumstr,numpathstr,isagin,aginw,aginx) 
    if IsNil(father) then
       return;
    end
    local splen =  father.transform.childCount;
...
function tjz_PushFun.repaceNum(args)
    if args == "." then
        return 10;
    elseif args == "w" then
        return 11;
...
--显示金钱 只是万和亿 用 w y
function tjz_PushFun.showNumberText2(numInt)
