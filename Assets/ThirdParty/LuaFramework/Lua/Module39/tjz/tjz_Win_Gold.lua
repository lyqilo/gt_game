...
function tjz_Win_Gold.setPer(args)
--   self.init();
--   self.per = args;
...
function tjz_Win_Gold.init(args)
    self.guikey = "cn";
    self.per = nil;
    self.winGoldroll = nil;
    self.addgold = 0;--增加的钱
...
function tjz_Win_Gold.winGoldRollCom(obj,args)
    self.per.gameObject:SetActive(false);
   if self.isclose == false then
      coroutine.start(function (args)
         ...
         tjz_Event.dispathEvent(tjz_Event.xiongm_gold_roll_com,self.ftar);
...
function tjz_Win_Gold.winGoldRollIng(obj,args)
    tjz_PushFun.CreatShowNum(self.moneycont,args,"win_",false,82,2,430,-200);
end

function tjz_Win_Gold.addEvent()
   --tjz_Event.addEvent(tjz_Event.xiongm_show_win_gold,self.showWinGold,self.guikey);
end

function tjz_Win_Gold.isDispathGameOver()
...
function tjz_Win_Gold.showWinGold(args)
   ...
   self.per.gameObject:SetActive(true);
    tjz_PushFun.CreatShowNum(self.moneycont,args.data.addgold,"win_",false,82,2,430,-200);
   coroutine.start(function()
        coroutine.wait(1);
        if tjz_Socket.isongame==false then
           return;
...
function tjz_Win_Gold.doShowWinGold(args)
...
function tjz_Win_Gold.removeEvent()
