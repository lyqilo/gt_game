...
function tjz_RunCont_Item:addEvent()
    ...
    tjz_Event.addEvent(tjz_Event.xiongm_init,tjz_Event.hander(self,self.gameInit),self.guikey);
end
...
function tjz_RunCont_Item:gameInit()
    for i=1,3 do
        openimg =  tjz_Data.selectImg[self.curindex+(3-i)*5];
        self.opendata[i] = openimg;
        self.showdata[i] = openimg;
...
function tjz_RunCont_Item:clientRunPer()
    local pos = nil;
    local py = 0;
...
function tjz_RunCont_Item:clientRunNext()
    local pos = nil;
    local py = 0;
    if self.speedtimer>self.maxtimer then
       self.speedtimer = self.maxtimer;
...
function tjz_RunCont_Item:serverRunPer()
    if self.runStopLen>0 then
    ...
    if self.isrun == false then
       ...
       self:randImg();
       self:setRunShow();
       local isscatter = self:playScatter();
       self.isperrun = false;
       if self.curindex==5 then
          tjz_Data.isruning = false;
       end
        tjz_Event.dispathEvent(tjz_Event.xiongm_run_com,{["curindex"] = self.curindex,["isscatter"] = isscatter });
    end
end

function tjz_RunCont_Item:serverRunNext()
...
function tjz_RunCont_Item:playScatter()
    ...
    elseif tjz_Data.byFreeCnt>0 and self.curindex==5 then
        for i=1,3 do
            openimg =  tjz_Data.selectImg[self.curindex+(3-i)*5];
            if openimg == tjz_Config.scatter then
               item = self.showtabel[i];
