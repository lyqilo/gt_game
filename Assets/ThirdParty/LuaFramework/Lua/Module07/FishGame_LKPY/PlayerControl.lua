...
function _CScoreColumn:SetScore(_score,_displayerTime)
    local count = (math.floor(_score*1.0/1500000*62)+1)*2;
    if count>40 then
        count = 40;
    end
    if count<2 then
...
function LockLine:Toward(startPoint,endPoint,len)
    ...
    if endPoint.x==startPoint.x then
        ...
        r=math.rad2Deg*math.atan((endPoint.y-startPoint.y)/(endPoint.x-startPoint.x));
    end
...
function _CPlayerControl:OnUserFire(_fire,_myChairId)
    ...
    if self._isOwner then 
		...
		--logError("我的子弹类型="..bulletType.."我的炮台类型="..paotaiType)
        --锟斤拷锟斤拷锟斤拷台
        self:setPaotaiType(paotaiType);
...
function _CPlayersControl:_onFire()
	...
	--logError("我的椅子ID="..self._myChairId)
...
function _CPlayersControl:GetCanLockFishByPos(_target)
    ...
    --local ray = main:ViewportPointToRay(_target);
    local t;  
    local ret,hitInfo;
    local it2,val2;
    while(fish) do
