flm_RunComp = {};
local self = flm_RunComp;

function flm_RunComp:new()
   local go = {};
   self.guikey = "cn";
...
function flm_RunComp:runStop()    
    self.iscom = true;
end

function flm_RunComp:setOpenPos()
...
function flm_RunComp:setPer(runargs)
   self:init();
...
function flm_RunComp:setSonItem(per)
    self.sonItem = per;
end

function flm_RunComp:randImg()
   ...
   for i=midindex,endindex do
     ...
     if sonfig==nil then
       ...
       if sonfig~=0 then
          self.runtabel[i].transform:Find("icon").gameObject:GetComponent("Image").sprite = flm_Data.icon_res.transform:Find(flm_Config.resimg_config[sonfig.src].normalimg).gameObject:GetComponent("Image").sprite;
       end
   end
end

function flm_RunComp:randFun(randtabel)
    local reslut = 0;   
...
--生成里面的图片
function flm_RunComp:createSonItem(num)
   local item = nil;
   self.createnum = num;
   for i = 1,num do
...
function flm_RunComp:clientRun()
   local pos = nil;
    local py = 0;
...
function flm_RunComp:serverRun()
   local pos = nil;
   local py = 0;
