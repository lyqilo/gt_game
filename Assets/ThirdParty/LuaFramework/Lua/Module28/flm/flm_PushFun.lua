flm_PushFun = {};
local self = flm_PushFun;

-- 创建数字Img显示对象
function flm_PushFun.CreatShowNum(father, getnumstr,numpathstr,isdanw,w,isagin,aginw,aginx) 
    ...
    for i = 1, string.len(numstr) do
        ...
        item = flm_Data.numres.transform:Find(numpathstr..prefebnum);
        ...
        if splen < i then
            ...
            go2.name = prefebnum;
            if prefebnum == "1" then
                go2.transform.localPosition = Vector3.New(klx - 6, 0, 0);
            end
        else
            ...
            if prefebnum == "1" then
                itemobj.transform.localPosition = Vector3.New(klx - 6, 0, 0);
            end
            -- end
        end
    end
...
function flm_PushFun.repaceNum(args)
    if args == "." then
        return 10;
    elseif args == "w" then
        return 11;
...
--显示金钱 只是万和亿 用 w y
function flm_PushFun.showNumberText2(numInt)
