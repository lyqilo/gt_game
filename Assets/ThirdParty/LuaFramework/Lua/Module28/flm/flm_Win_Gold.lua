...
self.ftar = nil;--���Ǹ�������
function flm_Win_Gold.setPer(args)
--   self.init();
--   self.per = args;
...
function flm_Win_Gold.init(args)
    self.guikey = "cn";
    self.per = nil;
    self.winGoldroll = nil;
    self.addgold = 0;--���ӵ�Ǯ
...
function flm_Win_Gold.winGoldRollCom(obj,args)
    self.per.gameObject:SetActive(false);
   if self.isclose == false then
      coroutine.start(function (args)
         ...
         flm_Event.dispathEvent(flm_Event.xiongm_gold_roll_com,self.ftar);
...
function flm_Win_Gold.winGoldRollIng(obj,args)
    flm_PushFun.CreatShowNum(self.moneycont,args,"win_",false,82,2,430,-200);
end

function flm_Win_Gold.addEvent()
   --flm_Event.addEvent(flm_Event.xiongm_show_win_gold,self.showWinGold,self.guikey);
end

function flm_Win_Gold.isDispathGameOver()
...
function flm_Win_Gold.showWinGold(args)
   ...
   self.per.gameObject:SetActive(true);
    flm_PushFun.CreatShowNum(self.moneycont,args.data.addgold,"win_",false,82,2,430,-200);
   coroutine.start(function()
        coroutine.wait(1);
        if flm_Socket.isongame==false then
           return;
...
function flm_Win_Gold.doShowWinGold(args)
...
function flm_Win_Gold.removeEvent()
