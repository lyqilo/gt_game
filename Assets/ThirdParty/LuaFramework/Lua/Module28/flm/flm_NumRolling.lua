...
-- callcom 运动完成
-- callupdate 中间更新
function flm_NumRolling:setdata(snum,enum,dtimer)
   ...
   self.isrun = true;
   self.onceStop = false;
   self.agenum = math.ceil((enum-snum)/dtimer);--平均值
   self.snum = snum;--开始值
   self.enum = enum;--结束值
   self.etimer = dtimer;--结束时间   
end

function flm_NumRolling:getisrun()
  return self.isrun;
end

function flm_NumRolling:setfun(callobj,callcom,callupdate)
   self.callobj = callobj;
   self.callcom = callcom;
   self.callupdate = callupdate;
end

function flm_NumRolling:update()
