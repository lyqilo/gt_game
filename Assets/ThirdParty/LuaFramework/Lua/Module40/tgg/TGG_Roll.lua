...
function TGG_Roll.StartRoll()
    self.rollIndex = 0;
    self.StopIndex = 0;
    self.timer = TGG_DataConfig.rollInterval;
    self.stopTimer = 0;
    self.startTimer = 0;
    self.currentState = 1;
    self.isstop = false;
end
function TGG_Roll.StopRoll()
    if self.isstop then
        return ;
    end
...
function TGG_Roll.Update()
    ...
    elseif self.currentState == 1 then
        ...
        for i = 1, self.rollIndex do
            self.rollList[i].verticalNormalizedPosition = self.rollList[i].verticalNormalizedPosition + Time.deltaTime * TGG_DataConfig.rollSpeed;--旋转
        ...
        if self.startTimer <= TGG_DataConfig.rollTime then
            --计算旋转时间，时间到就停止
    ...
    elseif self.currentState == 2 then
        ...
        if self.StopIndex < #self.rollList then
            ...
            if self.stopTimer >= TGG_DataConfig.rollInterval then
                self.stopTimer = 0;
                self.StopIndex = self.StopIndex + 1;
                --TODO 换正式结果图片
                self.ChangeResultIcon(self.StopIndex);
                self.rollList[self.StopIndex].verticalNormalizedPosition = TGG_DataConfig.rollDistance;
                self.SingleRollStop();
                if self.StopIndex == #self.rollList then
                    self.currentState = 0;
                    TGGEntry.OnStop();
...
function TGG_Roll.SingleRollStop()
    --TODO 音效
    TGG_Audio.PlaySound(TGG_Audio.SoundList.RS);
end
function TGG_Roll.ChangeIconRoll(index)
    local iconParent = self.rollList[index].content;
    for i = 0, iconParent.childCount - 1 do
        local iconIndex = math.random(1, 10);
        local changeIcon = TGGEntry.icons:Find(TGG_DataConfig.IconTable[iconIndex]):GetComponent("Image").sprite;
...
function TGG_Roll.ChangeResultIcon(index)
    local iconParent = self.rollList[index].content;
    for i = 0, iconParent.childCount - 1 do
        ...
        if i == iconParent.childCount - 1 then
            ...
            iconIndex = TGGEntry.ResultData.ImgTable[i * 5 + index];
        end
        local changeIcon = TGGEntry.icons:Find(TGG_DataConfig.IconTable[iconIndex + 1]):GetComponent("Image").sprite;
        img.sprite = changeIcon;
        if i ~= iconParent.childCount - 1 then
            iconParent:GetChild(i).gameObject.name = TGG_DataConfig.IconTable[iconIndex + 1];
        end
        img:SetNativeSize();
        local tempIcon = iconParent:GetChild(i):Find("Temp");
        if tempIcon ~= nil then
            iconIndex = math.random(0, 9);
            changeIcon = TGGEntry.icons:Find(TGG_DataConfig.IconTable[iconIndex + 1]):GetComponent("Image").sprite;
            tempIcon:GetComponent("Image").sprite = changeIcon;
            tempIcon:GetComponent("Image"):SetNativeSize();
        end
    end
