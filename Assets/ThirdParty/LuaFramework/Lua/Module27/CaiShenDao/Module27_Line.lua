...
function Module27_Line.Show()
    Module27_Audio.PlaySound(Module27_Audio.SoundList.LINE);
    self.showTable = {};
    self.showCount = {};
    self.showItemTable = {};
    self.currentShow = 0;
    self.showTime = 0;
    self.StartShowTime = 0;
    for i = 1, #Module27.ResultData.LineTypeTable do
        if Module27.ResultData.LineTypeTable[i][1] ~= 0 then
            local index = i;
            table.insert(self.showTable, index);
            local count = 0;
            for j = 1, #Module27.ResultData.LineTypeTable[i] do
                if Module27.ResultData.LineTypeTable[i][j] ~= 0 then
                    count = count + 1;
                end
            end
...
function Module27_Line.Update()
    if self.isWait then
        self.StartShowTime = self.StartShowTime + Time.deltaTime;
        if self.StartShowTime >= Module27_DataConfig.waitShowLineTime then
            self.StartShowTime = 0;
            self.isWait = false;
            self.ShowAll();
            self.isShow = true;
    ...
    if self.isShow then
        ...
        if self.currentShow == 0 then
            if self.showTime >= Module27_DataConfig.lineAllShowTime then
                self.showTime = 0;
            ...
            if self.showTime >= Module27_DataConfig.cyclePlayLineTime then
                self.showTime = 0;
                self.CloseSingle();
                self.currentShow = self.currentShow + 1;
                if self.currentShow > #self.showTable then
...
function Module27_Line.ShowAll()
    --显示总连线
    Module27.LineGroup.gameObject:SetActive(true);
    for i = 1, #self.showTable do
        Module27.LineGroup:GetChild(self.showTable[i] - 1).gameObject:SetActive(true);
        local showlist = Module27_DataConfig.Line[self.showTable[i]];--这是单条线对应位置
        local tempTable = {};--每组展示的元素集合
        for j = 1, self.showCount[i] do
            local index = showlist[j];
            local elem = Module27.ResultData.ImgTable[index];
            local column = math.ceil(index / 5);
            local raw = math.floor(index % 5);
            if raw == 0 then
                raw = 5;
            end
            local child = Module27.RollContent.transform:GetChild(raw - 1):GetComponent("ScrollRect").content:GetChild(column - 1);
            local icon = child:Find("Icon");
            if icon.childCount <= 0 then
                local go = self.CreateEffect(Module27_DataConfig.EffectTable[elem + 1]);
                go.transform:SetParent(child:Find("Icon"));
                go.transform.localPosition = Vector3.New(0, 0, 0);
                go.transform.localRotation = Quaternion.identity;
                go.transform.localScale = Vector3.New(1, 1, 1);
                go.gameObject:SetActive(true);
                go.name = Module27_DataConfig.EffectTable[elem + 1];
                if elem == 0 then
...
function Module27_Line.CloseAll()
    --关闭总显示
    for i = 1, #self.showTable do
        Module27.LineGroup:GetChild(self.showTable[i] - 1).gameObject:SetActive(false);
        for j = 1, #self.showItemTable[i] do
...
function Module27_Line.ShowSingle()
    --显示轮播
    Module27.LineGroup:GetChild(self.showTable[self.currentShow] - 1).gameObject:SetActive(true);
...
function Module27_Line.CloseSingle()
    --关闭单个显示
    Module27.LineGroup:GetChild(self.showTable[self.currentShow] - 1).gameObject:SetActive(false);
    for i = 1, #self.showItemTable[self.currentShow] do
        local icon = self.showItemTable[self.currentShow][i]:Find("Icon");
        icon:GetComponent("Image").enabled = true;
        if icon.childCount > 0 then
...
function Module27_Line.CollectEffect(obj)
    --回收动画
    if obj == nil then
        return ;
    end
    obj.transform:SetParent(Module27.effectPool);
    obj:SetActive(false);
end
function Module27_Line.CreateEffect(effectname)
    --创建动画，先从对象池中获取
    local go = Module27.effectPool:Find(effectname);
    if go ~= nil then
        return go.gameObject;
    end
    go = Module27.effectList:Find(effectname);
