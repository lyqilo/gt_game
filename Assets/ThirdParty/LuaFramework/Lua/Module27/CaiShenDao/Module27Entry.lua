...
require(g_sSlotModuleNum .. "Module27_Network")
require(g_sSlotModuleNum .. "Module27_Roll")
require(g_sSlotModuleNum .. "Module27_DataConfig")
require(g_sSlotModuleNum .. "Module27_Caijin")
require(g_sSlotModuleNum .. "Module27_Line")
require(g_sSlotModuleNum .. "Module27_Result")
require(g_sSlotModuleNum .. "Module27_Rule")
require(g_sSlotModuleNum .. "Module27_Audio")
...
function Module27:Awake(obj)
    ...
    self.FindComponent();
    self.AddListener();
    Module27_Roll.Init(self.RollContent);
    Module27_Audio.Init();
    Module27_Network.AddMessage();--添加消息监听
    Module27_Network.LoginGame();--开始登录游戏
    GameSetsPanel.CreateHideObj(self.transform, false);
    Module27_Audio.PlayBGM();
end
function Module27.OnDestroy()
    Module27_Line.Close();
    Module27_Network.UnMessage();
end
function Module27:Update()
    Module27_Roll.Update();
    Module27_Caijin.Update();
    Module27_Line.Update();
    Module27_Result.Update();
end
function Module27.FindComponent()
    ...
    self.winNormalEffect.gameObject:SetActive(false);
    self.CSJLEffect.gameObject:SetActive(false);
    self.CYGGEffect.gameObject:SetActive(false);
    self.JYMTEffect.gameObject:SetActive(false);
    self.EnterFreeEffect.gameObject:SetActive(false);
    self.CSJLResult.gameObject:SetActive(false);
...
function Module27.AddListener()
    ...
    for i = 1, #Module27_DataConfig.autoList do
        --自动次数选择
        local child = nil;
        ...
        if Module27_DataConfig.autoList[i] > 1000 then
            ...
            child:Find("Num"):GetComponent("TextMeshProUGUI").text = ShowRichText(Module27_DataConfig.autoList[i]);
        end
        child.gameObject.name = tostring(Module27_DataConfig.autoList[i]);
        child:GetComponent("Button").onClick:RemoveAllListeners();
        child:GetComponent("Button").onClick:AddListener(function()
            self.currentAutoCount = Module27_DataConfig.autoList[i];
...
function Module27.GetGameConfig()
    --获取远程端配置
    ...
    UnityWebRequestManager.Instance:GetText(url .. "Config/module27.json", 4, formdata, function(state, result)
        ...
        Module27_DataConfig.rollTime = data.rollTime;
        Module27_DataConfig.rollReboundRate = data.rollReboundRate;
        Module27_DataConfig.rollInterval = data.rollInterval;
        Module27_DataConfig.rollSpeed = data.rollSpeed;
        Module27_DataConfig.caijinGoldChangeRate = data.caijinGoldChangeRate;
        Module27_DataConfig.winGoldChangeRate = data.winGoldChangeRate;
        Module27_DataConfig.selfGoldChangeRate = data.selfGoldChangeRate;
        Module27_DataConfig.freeLoadingShowTime = data.freeLoadingShowTime;
        Module27_DataConfig.smallGameLoadingShowTime = data.smallGameLoadingShowTime;
        Module27_DataConfig.rollDistance = data.rollDistance;
        Module27_DataConfig.REQCaiJinTime = data.REQCaiJinTime;
        Module27_DataConfig.lineAllShowTime = data.lineAllShowTime;
        Module27_DataConfig.cyclePlayLineTime = data.cyclePlayLineTime;
        Module27_DataConfig.waitShowLineTime = data.waitShowLineTime;
        Module27_DataConfig.autoRewardInterval = data.autoRewardInterval;
        Module27_DataConfig.autoNoRewardInterval = data.autoNoRewardInterval;
    end);
end
function Module27.ToCharArray(num)
    --拆解字符串
...
function Module27.FormatNumberThousands(num)
    return num;
    --对数字做千分位操作
...
function Module27.ShowText(str)
...
function Module27.ReduceChipCall()
    --减注
    ...
    self.ChipNum.text = self.ShowText(self.FormatNumberThousands(self.CurrentChip * Module27_DataConfig.ALLLINECOUNT));
end
function Module27.AddChipCall()
    --加注
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
    if self.SceneData.chipList == nil or #self.SceneData.chipList <= 0 then
        return ;
    ...
    self.CurrentChip = self.SceneData.chipList[self.CurrentChipIndex];
    self.ChipNum.text = self.ShowText(self.CurrentChip * Module27_DataConfig.ALLLINECOUNT);
end
function Module27.StartGameCall()
    --开始游戏
    ...
    if self.isRoll then
        --TODO 急停
        Module27_Roll.StopRoll();
        return ;
    end
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
    if self.myGold < self.CurrentChip * Module27_DataConfig.ALLLINECOUNT then
    ...
    Module27_Network.StartGame();
end
function Module27.ClickMenuCall()
    --点击显示菜单
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
    ...
    self.menuTweener = self.menulist:Find("Content"):DOLocalMove(Vector3.New(0, 50, 0), 0.5):OnComplete(function()
...
function Module27.CloseMenuCall()
    --关闭菜单
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
    if self.menuTweener ~= nil then
        self.menuTweener:Kill();
...
function Module27.CloseGameCall()
...
function Module27.OnClickAutoItemCall()
    --点击选择自动次数
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
...
function Module27.ShowRultPanel()
    --显示规则
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
    Module27_Rule.ShowRule();
end
function Module27.SetMaxChipCall()
    --设置最大下注
    Module27_Audio.PlaySound(Module27_Audio.SoundList.BTN);
    self.CurrentChipIndex = #self.SceneData.chipList;
    self.CurrentChip = self.SceneData.chipList[self.CurrentChipIndex];
...
function Module27.CSJLRoll()
    Module27_Network.StartGame();
end
function Module27.StopAutoGame()
    --停止自动旋转
    self.isAutoGame = false;
...
function Module27.Roll()
    --开始转动    
    Module27_Audio.ResetEffect();
    Module27_Audio.PlaySound(Module27_Audio.SoundList.GO);
    if not self.isFreeGame and not Module27_Result.isWinCSJL then
        self.myGold = self.myGold - self.CurrentChip * Module27_DataConfig.ALLLINECOUNT;
        self.selfGold.text = self.ShowText(self.FormatNumberThousands(self.myGold));
    end
    self.WinNum.text = self.ShowText(0);
    Module27_Result.HideResult();
    Module27_Line.Close();
    Module27_Roll.StartRoll();
end
function Module27.OnStop()
    --停止转动
    log("停止")
    Module27_Result.ShowResult();
end
function Module27.InitPanel()
    --场景消息初始化
    ...
    self.ChipNum.text = self.ShowText(self.CurrentChip * Module27_DataConfig.ALLLINECOUNT);
    self.WinNum.text = self.ShowText(0);
    Module27_Caijin.isCanSend = true;
    self.isRoll = false;
    Module27_Line.Init();
    Module27_Result.Init();
    if self.SceneData.freeNumber > 0 then
        --如果免费
        self.freeCount = self.SceneData.freeNumber;
        self.isFreeGame = true;
        Module27_Audio.PlayBGM(Module27_Audio.SoundList.CSJLBGM);
        Module27_Result.ShowFreeEffect(true);
    end
end

function Module27.FreeRoll()
    --判断是否为免费游戏
    Module27_Result.isShow = false;
    if self.isFreeGame then  
        self.freeCount = self.freeCount - 1;
        if self.freeCount < 0 then
            --免费结束
            if Module27_Result.showFreeTweener ~= nil then
                Module27_Result.showFreeTweener:Kill();
                Module27_Result.showFreeTweener = nil;
            end
            self.FreeContent.gameObject:SetActive(false);
            self.isFreeGame = false;
            Module27_Result.isFreeGame=false
