FXGZ_DataConfig = {}

local self = FXGZ_DataConfig;
self.ServerDataURL = "http://127.0.0.1:80/module27.json"
self.CHIPLISTCOUNT = 10;--下注列表长度
self.CAISHENCOUNT = 10;--财神个数
self.ALLIMGCOUNT = 20;--图标个数
self.ALLLINECOUNT = 40;--连线数量
self.COLUMNCOUNT = 5;--每列个数
self.SMALLIMGCOUNT = 4;--小游戏图标个数
self.SMALLROLLCOUNT = 24;--小游戏外围图标个数

self.rollInterval = 0.16;--每列转动间隔时间
self.rollSpeed = 20;--每列转动速度,可调节快慢
self.rollTime = 1.5;--转动时间
self.rollReboundRate = 2;--每列停止后反弹比例（0：立即停止，值越大反弹速度越慢）
self.rollDistance = 110;--回弹距离，数值越大距离越大
self.winGoldChangeRate = 1;--得分跑分速率
self.winBigGoldChangeRate = 3.1;--得分跑分速率
self.selfGoldChangeRate = 2;--自身跑分速率
self.caijinGoldChangeRate = 10;--彩金跑分速率
self.freeLoadingShowTime = 3.3;--中免费展示时间
self.smallGameLoadingShowTime = 1.5;--中小游戏展示时间
self.smallGameRuleShowTime = 5;--中小游戏展示时间
self.REQCaiJinTime = 5;--请求彩金频率
self.lineAllShowTime = 2;--总连线展示时间
self.cyclePlayLineTime = 2;--连线轮播展示时间
self.waitShowLineTime = 0.3;--等待展示连线奖励时间
self.autoRewardInterval = 2;--自动或免费中奖间隔
self.autoNoRewardInterval = 0.2;--自动或免费未中奖间隔
self.wutiaoTime = 1;--自动或免费未中奖间隔
self.addSpeedTime = 3;
self.showSelectFreeResultTime = 2;
self.showFreeResultTime = 3;--展示免费结算界面时间
self.showSmallGameResultTime = 3;--展示小游戏结算界面时间
self.smallMoveSpeed = 0.05;--小游戏外围旋转速度

self.autoList = {--自动旋转次数
    10000, 100, 50, 20
}
self.WinConfig = {--结果描述
    "财源广进",
    "紫气东来",
    "财福俱进",
    "家财万贯",
    "恭喜发财",
    "六六大顺",
};
self.IconTable = {
    "Item1", -- j 1
    "Item2", -- q 2
    "Item3", --k  3
    "Item4", --a  4
    "Item5", --花  5
    "Item6", --玉佩  6
    "Item7", --锣  7
    "Item8", --灯笼  8
    "Item9", --鼓   9
    "Item10", --jackpot   10
    "Item11", --wild   11
    "Item12", --bonus 12
};
self.SmallIconTable = {
    "SmallItem1", -- j 1
    "SmallItem2", -- q 2
    "SmallItem3", --k  3
    "SmallItem4", --a  4
    "SmallItem5", --花  5
    "SmallItem6", --玉佩  6
    "SmallItem7", --锣  7
    "SmallItem8", --灯笼  8
    "SmallItem9", --鼓   9
    "SmallItem10", --jackpot   10
    "SmallItem11", --wild   11
};
self.oddList = {--赔率表
    { 40, 40, 45, 45, 80, 100, 160, 180, 200, 240, 0, 0 },
    { 20, 20, 25, 25, 40, 50, 80, 90, 100, 120, 0, 0 },
    { 5, 5, 8, 8, 20, 25, 40, 45, 50, 60, 0, 0 },
};
self.SmallIconPos = {
    { 6, 15, 21 }, --菠萝
    { 10, 16 }, --橘子
    { 2, 9, 14 }, --香蕉
    { 3, 11, 22 }, --樱桃
    { 12, 17, 23 }, --柠檬
    { 5, 18, 24 }, --葡萄
    { 4, 8, 20 }, --西瓜
    { 1, 7, 13, 19 } --炸弹
}
self.EffectTable = {
    "Item1", -- j 1
    "Item2", -- q 2
    "Item3", --k  3
    "Item4", --a  4
    "Item5", --花  5
    "Item6", --玉佩  6
    "Item7", --锣  7
    "Item8", --灯笼  8
    "Item9", --鼓   9
    "Item10", --jackpot   10
    "Item11", --wild   11
    "Item12", --bonus 12
};

self.GameResultType = {--游戏结果类型
    NORMAL = 0, --普通
    CYGG = 1, --财源滚滚
    JYMT = 2--金玉满堂
};
self.Line = {--连线结果
    { 1, 2, 3, 4, 5 }, --1
    { 1, 2, 8, 4, 5 }, --2
    { 1, 7, 8, 9, 5 }, --3
    { 1, 7, 13, 9, 5 }, --4
    { 1, 7, 3, 9, 5 }, --5
    { 1, 12, 8, 14, 5 }, --6
    { 1, 12, 13, 14, 5 }, --7
    { 1, 12, 18, 14, 5 }, --8
    { 1, 17, 13, 19, 5 }, --9
    { 1, 17, 18, 19, 5 }, --10
    { 6, 7, 8, 9, 10 }, --11
    { 6, 7, 3, 9, 10 }, --12
    { 6, 7, 13, 9, 10 }, --13
    { 6, 2, 3, 4, 10 }, --14
    { 6, 12, 13, 14, 10 }, --15
    { 6, 12, 18, 14, 10 }, --16
    { 6, 2, 8, 4, 10 }, --17
    { 6, 12, 8, 14, 10 }, --18
    { 6, 17, 13, 19, 10 }, --19
    { 6, 17, 18, 19, 10 }, --20
    { 11, 12, 13, 14, 15 }, --21
    { 11, 12, 8, 14, 15 }, --22
    { 11, 12, 18, 14, 15 }, --23
    { 11, 7, 8, 9, 15 }, --24
    { 11, 17, 18, 19, 15 }, --25
    { 11, 7, 3, 9, 15 }, --26
    { 11, 7, 13, 9, 15 }, --27
    { 11, 17, 13, 19, 15 }, --28
    { 11, 2, 8, 4, 15 }, --29
    { 11, 2, 3, 4, 15 }, --30
    { 16, 17, 18, 19, 20 }, --31
    { 16, 17, 13, 19, 20 }, --32
    { 16, 12, 13, 14, 20 }, --33
    { 16, 12, 8, 14, 20 }, --34
    { 16, 12, 18, 14, 20 }, --35
    { 16, 7, 13, 9, 20 }, --36
    { 16, 7, 8, 9, 20 }, --37
    { 16, 7, 3, 9, 20 }, --38
    { 16, 2, 8, 4, 20 }, --39
    { 16, 2, 3, 4, 20 }, --40
}
function FXGZ_DataConfig.GetEffectName(index)
    return self.EffectTable[index];
end