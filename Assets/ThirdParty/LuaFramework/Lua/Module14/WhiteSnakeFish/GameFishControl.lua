...
function _CGameFishControl:ctor(_sceneControl)
    ...
    --本地ID
    self._localIdsMap   = map:new();

    --缓存本地的鱼的ID
    self._localIdsVec   = vector:new();
    
    --申请一个ID段
    for i=1,50,1 do
        self._localIdsMap:insert(i,1);
        self._localIdsVec:push_back(i);
    end
...
--创建本地鱼
function _CGameFishControl:CreateLocalFish(_type,_creator,_flag)
    _flag = _flag or G_GlobalGame.Enum_FISH_FLAG.Common_Fish;
    local vec = self._cacheFishMap:value(_type);
    local fish;
    local fishId = self._localIdsVec:pop_front();
    if vec==nil or vec:size()==0 then
        fish = CFish.Create(_type,_creator);
    else
        fish = vec:pop();
...
function _CGameFishControl:Update(_dt)
    ...
    while(val) do
        ...
        if fish then
            fish:Cache();
            if self._localIdsMap:find(val) then
                --存储本地ID缓存
                self._localIdsVec:push_back(val);
            end
...
function _CGameFishControl:GetNextLockFish(_fish)
    ...
    while (val) do
        ...
        if isStart then
            if not fish:IsDie() and fish:IsCanBeLock() then
                return fish;
            end
        else
            if fish:FishID() == _fish:FishID() then
