...
local _CPauseScreenEffect = class("_CPauseScreenEffect",_CEffect);
...
function _CPauseScreenEffect:_init(_type)
    ...
    self.shadows = {};
    self._shadowTransform = self.transform:Find("Shadows");
    local childCount =  self._shadowTransform.childCount;
    local child;
    for i=childCount,1,-1 do
        child = self._shadowTransform:GetChild(i-1);
        self.shadows[i] = child;
        child.gameObject:SetActive(false);
    end
    self.shadowCount = childCount;
end

function _CPauseScreenEffect:Init()
    self._runTime = 0;
    self.transform.localScale = Vector3.New(2.5,2.5,2.5);
    for i=1,self.shadowCount do
        self.shadows[i].gameObject:SetActive(false);
    end
    --解决定海翻屏导致显示翻转
    self.transform.rotation = Quaternion.Euler(0,0,0);
    self.scaleSpeed = 3.4;
    self.timeDisappear = 0.05;
    self.localScale = 2;
    --正在缩放的过程中
    self.isScale = true;
    --是否在显示影子
    self.isDisplayShadow = true;
    --停止缩小的时间
    self._pauseScaleTime=0;
end
...
function _CPauseScreenEffect:Update(_dt)
    ...
    if self.isScale then
        self.localScale = self.localScale - self.scaleSpeed * _dt;
        self.scaleSpeed = self.scaleSpeed -0.06;
        if self.scaleSpeed<0.7 then
            self.scaleSpeed = 1;
        end
        if self.localScale <= 1 then
            self.localScale = 1;
            self.isScale = false;
            self._pauseScaleTime = self._runTime;
            self.timeDisappear = self.timeDisappear-0.02;
        end
        self.transform.localScale = Vector3.New(self.localScale,self.localScale,self.localScale);
        if self.isDisplayShadow then
            local count = math.floor(self._runTime/self.timeDisappear);
            count = count>self.shadowCount and self.shadowCount or count;
            for i=1,count do
                self.shadows[self.shadowCount - i + 1 ].gameObject:SetActive(true);
            end
            if count >= self.shadowCount then
                self.isDisplayShadow = false;
            end
        end
    else
        if not self.isDisplayShadow then
            local leftTime = self._runTime - self._pauseScaleTime;
            local count = math.floor(leftTime/self.timeDisappear);
            count = count>self.shadowCount and self.shadowCount or count;
            for i=1,count do
                self.shadows[i].gameObject:SetActive(false);
            end
    ...
    if self._runTime >G_GlobalGame.GameConfig.SceneConfig.pauseScreenTime then
        --定时时间
        self:SendEvent(G_GlobalGame.Enum_EventID.EffectDisappear);
    end
end
...
function _CEffectControl:ctor(_sceneControl)
    ...
    G_GlobalGame:RegEventByStringKey("NotifyCreatePauseScrn",self,self.OnCreatePauseScreenEffect);
...
function _CEffectControl:OnCreatePauseScreenEffect(_eventId,_eventData)
    ...
    while(val) do
        ...
        if effect then
            if effect:Type() == _type then
                self._delEffectVec:push_back(val);
            end
        end
        val = it();
    end
