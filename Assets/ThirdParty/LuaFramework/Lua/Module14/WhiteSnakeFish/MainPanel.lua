...
local function Include()
    ...
    --通用
    GamePacketName("Module14/WhiteSnakeFish")
...
function MainPanel:Awake(obj)
    ...
    --执行异步加载
    MainPanel:InitGame(obj)
end

function MainPanel.Start(obj)
end

function MainPanel:InitGame(obj)
    local mainPanel = G_GlobalGame._mainPanel
    ...
    G_GlobalGame._canvasPanel.localPosition = Vector3.New(0, 0, 0) --Vector3.New(0,0,0);
...
function MainPanel:AsyncInitGame(obj)
    local mainPanel = G_GlobalGame._mainPanel
    --自己游戏窗口对象
    G_GlobalGame._gamePanelObj = obj
    ...
    --设置创建器
    G_GlobalGame._animaTool:SetCreator(handler(G_GlobalGame._goFactory, G_GlobalGame._goFactory.getCommonSprite))
    local asyncLoad = function(go)
        G_GlobalGame._canvasPanelGO = go
        G_GlobalGame._canvasPanel = G_GlobalGame._canvasPanelGO.transform
        G_GlobalGame._canvasPanel:SetParent(obj.transform)
        ...
        mainPanel:Init()
        mainPanel:InitSystem()
        G_GlobalGame:StartAsyncLoad()
    end

    G_GlobalGame._canvasPanelGO = G_GlobalGame._goFactory:createMainPanel(asyncLoad)
    G_GlobalGame:AsyncLoad()
end
...
function MainPanel.Update()
    if
    G_GlobalGame.RunningPlatform == RuntimePlatform.WindowsPlayer or
    G_GlobalGame.RunningPlatform == RuntimePlatform.WindowsEditor or
    G_GlobalGame.RunningPlatform == RuntimePlatform.WindowsWebPlayer or
    G_GlobalGame.IsEditor
    then
        local _dt = Time.deltaTime
        if G_GlobalGame or G_GlobalGame.isQuitGame then
            if G_GlobalGame:IsAsyncLoad() then
                -- 进行异步加载
                G_GlobalGame:AsyncLoading(_dt)
            else
