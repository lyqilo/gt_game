...
function _CFishCollider:_initStyle(_type)
    ...
    if childCount==0 then
        self._colliders[1] = self.gameObject:GetComponent(typeof(UnityEngine.BoxCollider));
    else
        local transform;
        for i=1,childCount do
            transform = self.transform:GetChild(i-1);
            self._colliders[i] = transform.gameObject:GetComponent(typeof(UnityEngine.BoxCollider));
        end
    end
end

function _CFishCollider:Colliders()
...
function _CFish:_move(_dt)
    if self.straightMove then
        ...
        self.transform.localPosition = self.beginStraightPos + self.speed *self.straightMoveTime;
        return;
    end
...
--    self.runRotationTime = self.bgRotationTime + self.runRotationTime;
    local fishCount = #self.fishActions;
    for i=1,fishCount do
        self.fishActions[i]:_bgRotate(self.rotation * _dt);
    end
end

--function _CFish:CorrentCollider()
...
function _CFish:Die()
    local fishConfig = G_GlobalGame.GameConfig.FishInfo[self._fishType];
    if fishConfig.deadEffect==nil or fishConfig.deadEffect==0 then
    else
...
--        LineSource  
--        PauseScreen 
        local  Bit = G_GlobalGame.FunctionsLib.Bit;
        if Bit:_and(fishConfig.deadEffect,BombEffectType.FishDead)~=0 then
            --特效
            G_GlobalGame:DispatchEventByStringKey("NotifyCreateEffect",{type=BombEffectType.FishDead,position = self:Position()});  
        end
        if Bit:_and(fishConfig.deadEffect,BombEffectType.JuBuBomb)~=0 then
            --特效
            G_GlobalGame:DispatchEventByStringKey("NotifyCreateEffect",{type=BombEffectType.JuBuBomb,position = self:Position()});  
        end
        if Bit:_and(fishConfig.deadEffect,BombEffectType.QuanPing)~=0 then
            --特效
            G_GlobalGame:DispatchEventByStringKey("NotifyCreateEffect",{type=BombEffectType.QuanPing,position = self:Position()});  
        end
...
--    if(self._fishType == FishType.FISH_KIND_42 ) then
--        G_GlobalGame:PlayEffect(G_GlobalGame.SoundDefine.CatchFish_9);
--    elseif( self._fishType == FishType.FISH_KIND_41 ) then
--        G_GlobalGame:PlayEffect(G_GlobalGame.SoundDefine.CatchFish_8);
--    elseif( self._fishType == FishType.FISH_KIND_20 ) then
--        G_GlobalGame:PlayEffect(G_GlobalGame.SoundDefine.CatchFish_7);
--    elseif (self._fishType>FishType.FISH_KIND_17 and self._fishType< FishType.FISH_KIND_20) or 
--        self._fishType==FishType.FISH_KIND_41 or self._fishType==FishType.FISH_KIND_42 
--        or self._fishType==FishType.FISH_KIND_22 then
--        --鱼死亡的声音
--        local index = math.random(G_GlobalGame.SoundDefine.CatchFish_1,G_GlobalGame.SoundDefine.CatchFish_6);
--        G_GlobalGame:PlayEffect(index);
--    elseif (self._fishType>=FishType.FISH_KIND_23 and self._fishType<=FishType.FISH_KIND_24) 
--        or  self._fishType==FishType.FISH_KIND_21 then
--        --播放炸弹声音
--        G_GlobalGame:PlayEffect(G_GlobalGame.SoundDefine.Bomb);
--    end
end
...
function _CFish:StraightMove(multiple)
    ...
    --]]
    y = math.sin(rd)*fishConfig.iSpeed*multiple*GameConfig.SceneConfig.FrameCount;
    x = math.cos(rd)*fishConfig.iSpeed*multiple*GameConfig.SceneConfig.FrameCount; 
