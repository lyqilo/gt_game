...
{
    private static AspectRatioController m_Instance;
    ...
    public static AspectRatioController Instance
    {
        get
        {
            ...
            m_Instance = FindObjectOfType<AspectRatioController>();
            if (m_Instance == null)
            {
                m_Instance = new GameObject($"{nameof(AspectRatioController)} Singleton")
                    .AddComponent<AspectRatioController>();
            }
    ...
    // 最小值和最大值的窗口宽度/高度像素
    [SerializeField]
    private int minWidthPixel = 128;
    [SerializeField]
    private int minHeightPixel = 128;
    [SerializeField]
    ...
    void Start()
    {
        ...
        pixelWidthOfCurrentScreen = Screen.width;
        pixelHeightOfCurrentScreen = Screen.height;
