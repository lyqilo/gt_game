...
public class Packager
{
    ...
    public static void BuildAssetResource(BuildTarget target)
    {
        ...
        if (Directory.Exists(AppDataPath + "/StreamingAssets/lua/"))
            Directory.Delete(AppDataPath + "/StreamingAssets/lua/", true);
        AssetDatabase.Refresh();
    }
    ...
    static void HandleLuaBundle()
    {
        ...
        for (int i = 0; i < srcDirs.Length; i++)
        {
            if (AppConst.LuaByteMode)
            {
                ...
                for (int j = 0; j < files.Length; j++)
                {
                    str.Clear();
                    str.Append("out");
                    if (srcDirs[i].Equals(CustomSettings.luaDir))
                    {
                        str.Append("/lua_");
                    }
                    else if (srcDirs[i].Equals(CustomSettings.toluaLuaDir))
                    {
                        str.Append("/outlua_");
                    }
                    string middlepath = files[j].Substring(sourceDir.Length);
                    middlepath = middlepath.Replace("/", "_");
                    middlepath = middlepath.Replace("\\", "_");
                    if (middlepath.StartsWith("_"))
                    {
                        middlepath = middlepath.Substring(1);
                    }
                    str.Append(middlepath);
                    string dest = streamDir + str.ToString() + ".bytes";
        ...
        for (int i = 0; i < dirs.Length; i++)
        {
            ...
            AddBuildMap(AppConst.LuaAssetName, "*.bytes", path);
        }
        AddBuildMap(AppConst.LuaAssetName, "*.bytes", "Assets/" + AppConst.LuaTempDir);
    ...
    {
        string scenePath = AppDataPath + "/Scene/";
